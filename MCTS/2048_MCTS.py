...
class Node:
    ...
    def get_min_diff(self):
        ...
        for child in self.children:
            if max_visits - child.visits < min_diff and child.visits != max_visits:
    ...
    def rollout(self):
        ...
        while True:
            ...
            if temp_board.game_is_over() or turn_count == 15:
...
def expand_node(test_node, time_start, max_time, max_diff):
    ...
    while time.time() - time_start <= max_time:
        ...
        # if one node has been visited significantly more use it instead of waiting for time
        min_diff = test_node.get_min_diff()
        if min_diff >= max_diff and min_diff != 99999999999:
            print("min_diff: {}".format(min_diff))
            break
        elif to_exp_child_ind != 99999:
            to_exp_child = test_node.children[to_exp_child_ind]
            take_next_step(to_exp_child)
        else:
            break
...
def run(exploration_num, max_time, max_diff):
    ...
    turns = 0
    while not test_board.game_is_over() and turns < 20:
        ...
        expand_node(test_node, time_start, max_time, max_diff)
...
if __name__ == "__main__":
    ...
    max_diff = 300
    run(exploration_num, max_time, max_diff)
    pr.disable()
    pr.print_stats()
